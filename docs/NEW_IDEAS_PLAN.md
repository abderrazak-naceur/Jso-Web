...
### B24. Streaming a pagamento della singola partita (pay-per-view / paywall)
- **Obiettivo/valore:** quando c'è una partita i tifosi passano dal sito del club e pagano per vedere la diretta di quella singola gara (un biglietto digitale di visione / pay-per-view), oppure l'accesso è incluso in una membership. È l'accesso a pagamento alla SINGOLA diretta, distinto dal bundle stagionale streaming del [BUSINESS_PLAN](BUSINESS_PLAN.md) (sez. 6 idea 1, abbonamento a tutta la stagione) e dall'infrastruttura live editoriale/dati del match center di [PLATFORM_VISION_2030](PLATFORM_VISION_2030.md) 4.9 (che è redazionale e non a pagamento): qui si vende l'accesso alla ripresa di una specifica partita.
- **Dominio/API/frontend:** nuova entità `MatchStream` (`Id`, `MatchId`, `Provider` [YouTube/altro], `StreamUrl`/`PlaybackRef`, `IsPaid`, `Price` `numeric(14,2)`, `StartsAt`, `EndsAt`, `VisibilityAfterPurchase`) collegata a `Match`, e un record di acquisto/accesso per verificare chi ha diritto all'accesso: da riusare o estendere l'entità `MatchAccessPurchase` già proposta in [FAN_ACCOUNTS_PLAN](FAN_ACCOUNTS_PLAN.md) (`FanUserId`, `MatchId`, `Status`, `Amount`, `Currency`, `ProviderRef`, indice univoco su `FanUserId`+`MatchId`) invece di introdurre una tabella acquisti parallela, dato che un flusso di pagamento reale richiede i campi più ricchi che quella definisce (es. `Status`, `ProviderRef`). Admin per configurare lo streaming della partita sotto `[Authorize(Roles="MatchManager,ClubAdmin")]` (es. `POST/PUT /api/admin/matches/{id}/stream`); endpoint pubblico/fan per acquistare e ottenere l'accesso verificato lato server (es. `POST /api/fan/matches/{id}/stream/purchase` e `GET /api/fan/matches/{id}/stream` che restituisce il link/embed solo dopo pagamento verificato). Frontend: riuso del design system, stati loading/vuoto/errore, player/embed sbloccato solo dopo l'acquisto. Campi soldi in `numeric(14,2)`, migration PostgreSQL versionata, campi aggiunti nullable o con default, API-first pensata anche per Flutter.
- **Impatto e rischio/costo:** ricavo diretto potenzialmente alto, ma con nodi critici da chiarire prima. (a) DIRITTI di ripresa e trasmissione della partita necessari (accordo con lega/controparti). (b) PAGAMENTI mai gestiti sui server del club: nessun dato di carta transita dalla nostra infrastruttura, si usa un provider certificato (il nodo pagamenti/gateway TND descritto nel [BUSINESS_PLAN](BUSINESS_PLAN.md)). (c) Con YouTube si può usare una diretta non in elenco/privata sbloccata solo dopo pagamento verificato lato server, con l'avvertenza onesta che il controllo accessi via link NON è vera protezione DRM (resta il rischio di condivisione del link). (d) Costi di banda/CDN se si trasmette in proprio, oppure dipendenza dalla piattaforma YouTube. Privacy/GDPR sull'account tifoso e sulle transazioni (consenso, informativa, minimizzazione, audit sulle scritture). L'idea si abilita pienamente solo dopo un match center/live affidabile ([PLATFORM_VISION_2030](PLATFORM_VISION_2030.md) 4.9) e la risoluzione del nodo pagamenti.
- **Collegamento:** orizzonte O2/O3; presuppone l'account tifoso (`FanUser`/ruolo `Fan`) proposto in [FAN_ACCOUNTS_PLAN](FAN_ACCOUNTS_PLAN.md), non ancora implementato (idea fan-dipendente, bloccata su quel prerequisito). Si differenzia dal bundle stagionale streaming (sez. 6) e dalla riga costi streaming del [BUSINESS_PLAN](BUSINESS_PLAN.md) e dall'infrastruttura live editoriale di [PLATFORM_VISION_2030](PLATFORM_VISION_2030.md) 4.9: qui è l'accesso a pagamento alla singola diretta, non un abbonamento stagionale né un'infrastruttura live redazionale.
...
| B24 | Streaming pay-per-view partita | Monetizzazione | Alto | Alto | O2/O3 | Diritti + gateway pagamenti + banda/CDN |
...
2. **Selezionare i quick win** (costo zero, nessuna dipendenza dai pagamenti) come primi candidati, senza anticipare aree che richiedono account tifoso o gateway di pagamento non ancora risolti (vedi nodo pagamenti nel [BUSINESS_PLAN](BUSINESS_PLAN.md)). Per esempio B24 (streaming pay-per-view) NON è un quick win: dipende dai diritti di trasmissione e dal gateway pagamenti, quindi va rimandata a dopo la risoluzione di quei nodi.
