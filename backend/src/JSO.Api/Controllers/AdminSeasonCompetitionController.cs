...
public sealed class AdminSeasonCompetitionController(JsoDbContext db, AuditService audit) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateSeason(Guid id, SeasonRequest request, CancellationToken ct)
    {
        var season = await db.Seasons.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (season is null) return NotFound();
        if (string.IsNullOrWhiteSpace(request.Name) || request.Name.Trim().Length > 120)
            return BadRequest(new { message = "Season name is required and must be at most 120 characters." });

        var name = request.Name.Trim();
        if (await db.Seasons.AnyAsync(x => x.Id != id && x.Name.ToLower() == name.ToLower(), ct))
            return Conflict(new { message = "A season with this name already exists." });

        season.Name = name;
        season.IsActive = request.IsActive;
        await db.SaveChangesAsync(ct);
        await audit.LogAsync("UPDATE", "Season", season.Id.ToString(), User.FindFirst("sub")?.Value,
            User.FindFirst("email")?.Value, HttpContext.Connection.RemoteIpAddress?.ToString(), ct: ct);
        return Ok(season);
    }
    ...
    public async Task<IActionResult> DeleteSeason(Guid id, CancellationToken ct)
    {
        var season = await db.Seasons.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (season is null) return NotFound();
        if (await db.Matches.AnyAsync(x => x.SeasonId == id, ct))
            return Conflict(new { message = "This season is referenced by one or more matches and cannot be deleted." });

        db.Seasons.Remove(season);
        await db.SaveChangesAsync(ct);
        await audit.LogAsync("DELETE", "Season", id.ToString(), User.FindFirst("sub")?.Value,
            User.FindFirst("email")?.Value, HttpContext.Connection.RemoteIpAddress?.ToString(), ct: ct);
        return NoContent();
    }
    ...
    public async Task<IActionResult> UpdateCompetition(Guid id, CompetitionRequest request, CancellationToken ct)
    {
        var competition = await db.Competitions.SingleOrDefaultAsync(x => x.Id == id, ct);
        ...
        var name = request.Name.Trim();
        if (await db.Competitions.AnyAsync(x => x.Id != id && x.Name.ToLower() == name.ToLower(), ct))
            return Conflict(new { message = "A competition with this name already exists." });

        competition.Name = name;
        competition.Country = string.IsNullOrWhiteSpace(request.Country) ? null : request.Country.Trim();
        await db.SaveChangesAsync(ct);
        await audit.LogAsync("UPDATE", "Competition", competition.Id.ToString(), User.FindFirst("sub")?.Value,
            User.FindFirst("email")?.Value, HttpContext.Connection.RemoteIpAddress?.ToString(), ct: ct);
        return Ok(competition);
    }

    [HttpDelete("competitions/{id:guid}")]
    public async Task<IActionResult> DeleteCompetition(Guid id, CancellationToken ct)
    {
        var competition = await db.Competitions.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (competition is null) return NotFound();
        if (await db.Matches.AnyAsync(x => x.CompetitionId == id, ct))
            return Conflict(new { message = "This competition is referenced by one or more matches and cannot be deleted." });

        db.Competitions.Remove(competition);
        await db.SaveChangesAsync(ct);
        await audit.LogAsync("DELETE", "Competition", id.ToString(), User.FindFirst("sub")?.Value,
            User.FindFirst("email")?.Value, HttpContext.Connection.RemoteIpAddress?.ToString(), ct: ct);
        return NoContent();
    }
}
